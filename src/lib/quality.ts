// src/lib/quality.ts
//
// Analyses a document image and returns quality metrics used to decide
// whether the image is suitable for OCR.
...
export async function analyzeDocumentQuality(dataUrl: string): Promise<DocumentQuality> {
  ...
  // ── Grayscale + glare ─────────────────────────────────────────────────────
  const grays      = new Array<number>(totalPixels);
  let glarePixels  = 0;
  ...
  const edgeDensity = Number((strongEdgeCount / Math.max(1, (width - 2) * (height - 2))).toFixed(4));
  const glareRatio  = Number((glarePixels / totalPixels).toFixed(4));
  const aspectRatio = Number((width / Math.max(1, height)).toFixed(4));

  // ── Evaluations ───────────────────────────────────────────────────────────
  const blurStatus: "good" | "ok" | "bad" =
    blurScore >= BLUR_GOOD ? "good"
    : blurScore >= BLUR_BAD  ? "ok"
    :                          "bad";

  const looksSharpEnough  = blurStatus !== "bad";
  const looksBrightEnough = brightness >= MIN_BRIGHTNESS && contrast >= MIN_CONTRAST;
